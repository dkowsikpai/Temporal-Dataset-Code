...
OUT_DIR = BASE_DIR + "_hard_range_based_mcq_prompt"
...
for dir in tqdm(dirs):
    ...
    for json in tqdm(jsons, leave=False):
        with jsonlines.open(json) as reader:
            ...
            for obj in reader:
                ...
                for idx in range(K, len(obj["answer"])):
                    ...
                    if "[start date]" in item["query"]:
                        ...
                        item["query"] = f"In range {first_year}-{last_year}, {item['query']}? What is the mean value out of the option?"
                    ...
                    # Mean of the window of the answers
                    mean = []
                    for ans in answers:
                        if first_year <= int(ans["date"]) <= last_year:
                            mean.append(float(ans["answer"]))
                    mean = sum(mean) / len(mean)

                    item["answer"] = str(mean)
            ...
            per_category_counter[dir] = per_category_counter.get(dir, 0) + data.shape[0]
