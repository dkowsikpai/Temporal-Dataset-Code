...
OUT_DIR = BASE_DIR + "_hard_min_max_based_mcq_prompt"
...
def get_options(idx, mn, mx):
    options = []
    options.append((mn, mx))
    ...
    while len(options) < 4:
        ...
        for num in [mn, mx]:
            if is_number(num):
                if "." in str(num):
                    after_decimal = len(str(num).split(".")[1])
                else:
                    after_decimal = 0
                ...
                tp.append("0")
        options.append(tuple(tp))
       
    # Round off the options
    options = [(round(float(x[0]), 2), round(float(x[1]), 2)) for x in options]
    correct_option = options[0]
    random.shuffle(options)
    labeled_options = []
    for i in range(len(options)):
        label = "(" + chr(97 + i) + ") "
        labeled_options.append(label + str(options[i]).strip("()"))
            ...
            for obj in reader:
                ...
                for idx in range(K, len(obj["answer"])):
                    ...
                    if "[start date]" in item["query"]:
                        ...
                        item["query"] = f"In range {first_year}-{last_year}, {item['query']}? What is the minimum and maximum value out of the options?"
                    ...
                    options, correct_option = get_options(idx, mn, mx)
                    # pprint(options)
